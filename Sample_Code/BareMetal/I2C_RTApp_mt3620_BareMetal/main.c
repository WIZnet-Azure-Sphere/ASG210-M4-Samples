...
bool i2c_Write(uint8_t slavaddr)
{
	...
	int ret = mtk_os_hal_i2c_write(i2c_master_port_num, slavaddr, wbuf, sizeof(wbuf));
	if (I2C_OK == ret)
	{
	...
	{
		printf("I2C Send slavaddr 0x%x Failed\r\n", slavaddr);
		return false;
	}

	ret = mtk_os_hal_i2c_read(i2c_master_port_num, slavaddr, rbuf, sizeof(rbuf));
	if (I2C_OK == ret)
	{
...
{
	printf("%c", mtk_os_hal_uart_get_char(uart_port_num));
}
